...
//loop thru array of movies for initial buttons
       for (var i=0; i<topics.length; i++) {
        var button=$("<button>").attr("id", i).text(topics[i]).css({"padding": "1%", "font-size": "100%"});
        ...
        .done(function(response) {
          ...
          // Looping through each result item from gathered data
            ...
            // Setting the src attribute of the image to a property pulled off the result item
                image.attr({"src": results[i].images.fixed_height.url, "data-still": results[i].images.fixed_height_still.url, "data-animate": results[i].images.fixed_height.url, "data-state": "animate"}).addClass("gif").css("width", "60%"); 
